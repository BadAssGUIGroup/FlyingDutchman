...
BeerApp.initBox = function() {
    ...
    var faces = [
        ...
        [[4, 1], [5, 1], [6, 1], [7, 1]], // back
        [[2, 2], [6, 2], [7, 2], [3, 2]], // top
        [[0, 3], [1, 3], [5, 3], [4, 3]], // bottom
        [[1, 4], [5, 4], [6, 4], [2, 4]], // right
        [[0, 5], [3, 5], [7, 5], [4, 5]]  // left
    ];
    ...
    var boxModel = new Model(mesh, vec3.set(vec3.create(), 2, 1, 1.5));
...
BeerApp.initBeerMug = function(sides) {
    ...
    for (var i = 0; i < n; i++) {
        ...
        beerTop.push([r * Math.sin(i * alpha) / 1.2, h / 1.7, r * Math.cos(i * alpha) / 1.2]);
...
BeerApp.render = function() {
    ...
    _.forEach(this.opaqueModels, function (model) {
        model.render(this.pipeline.uVMatrix, ShaderPrograms.lightingProgram);
    }, this);

    if (this.beer.height > 0) {
        this.beerModel.render(this.pipeline.uVMatrix, ShaderPrograms.lightingProgram);
    }
